...
function getPolygonStyle(feature: CampusFeature, selected: boolean) {
  if (feature.category === 'building') {
    return {
      color: '#b45309',
      fillColor: '#facc15',
      weight: selected ? 3.8 : 1.8,
        ...
        {features.map((feature) => {
          ...
          return (
              <Polygon
                  ...
                  pathOptions={getPolygonStyle(feature, selected)}
                  eventHandlers={{
                    click: () => onFeatureClick(feature.id),
                    mouseover: (event) => {
                      if (!selected) {
                    ...
                    mouseout: (event) => {
                      if (!selected) {
                        event.target.setStyle(getPolygonStyle(feature, false))
                      }
                    },
                  }}
              >
